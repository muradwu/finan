...
export async function GET() {
  ...
  try {
    ...
    const [settings, currentMonthTx, prevMonthTx, last30DaysTx, recentTx] = await Promise.all([
      ...
      prisma.transaction.findMany({ where: { userId, date: { gte: prevMonthStart, lte: prevMonthEnd } } }),
      prisma.transaction.findMany({ where: { userId, date: { gte: thirtyDaysAgo }, type: "expense" } }),
    ...
    const byCategory = currentMonthTx.filter((t) => t.type === "expense").reduce<Record<string, number>>((acc, t) => {
      acc[t.category] = (acc[t.category] ?? 0) + t.amount
      return acc
    }, {})

    const prevByCategory = prevMonthTx.filter((t) => t.type === "expense").reduce<Record<string, number>>((acc, t) => {
      acc[t.category] = (acc[t.category] ?? 0) + t.amount
      return acc
    }, {})

    const topCategories = Object.entries(byCategory).sort(([, a], [, b]) => b - a).slice(0, 3).map(([category, amount]) => ({ category, amount }))
    ...
    for (const t of currentMonthTx.filter((t) => t.type === "income")) {
      const key = new Date(t.date).toISOString().split("T")[0]
      if (dailyData[key]) dailyData[key].income += t.amount
    }

    const chartData = Object.entries(dailyData).map(([date, vals]) => ({ date, ...vals }))
    const daysLeft = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate() - now.getDate()

    return NextResponse.json({
      budget, currency, remaining, currentExpenses, currentIncome,
      byCategory, prevByCategory, topCategories, avgTransaction, avgTrend,
      chartData, recentTransactions: recentTx, daysLeft, transactionCount: currentMonthTx.length,
    })
  } catch (err) {
    console.error(err)
