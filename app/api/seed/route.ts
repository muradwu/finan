...
const seedData: Array<{
  amount: number; currency: string; category: Category
  description: string; date: Date; type: TransactionType
}> = [
  ...
  { amount: 12.0,  currency: "AZN", category: "food",          description: "Пицца, доставка Wolt",         date: daysAgo(7),  type: "expense" },
...
export async function POST(_req: NextRequest) {
  ...
  try {
    ...
    await prisma.transaction.createMany({ data: seedData.map((t) => ({ ...t, userId })) })

    await prisma.settings.upsert({
      where: { userId },
      update: { monthlyBudget: 1500, currency: "AZN" },
      create: { userId, monthlyBudget: 1500, currency: "AZN", theme: "system" },
    })
